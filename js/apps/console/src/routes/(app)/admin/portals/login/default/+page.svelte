<script lang="ts">
  ...
  async function saveDefault() {
    ...
    feedback = null;
    try {
      const response = await trellis.request("Auth.SetLoginPortalDefault", {
        portalId: optionToPortalId(defaultDraft),
      } satisfies AuthSetLoginPortalDefaultInput).take();
      if (isErr(response)) { error = errorMessage(response); return; }
      feedback = "Default login portal updated.";
...
<section class="space-y-4">
  <PageToolbar title="Default login portal" description="Set the fallback portal used for login flows.">
    {#snippet actions()}
  ...
  <Panel title="Set default" eyebrow="Login portal">
    {#if loading}
      <LoadingState label="Loading login portal default" />
    {:else}
