...
Deno.test({
  ...
  try {
    ...
    await t.step("publish() -> event() receives and acks", async () => {
      ...
      const received: Array<{ header: { id: string; time: string }; foo: string }> = [];
      ...
      await waitFor(() => received.length === 1, { description: "event delivery" });
    ...
    await t.step("handler error naks and redelivers", async () => {
      ...
      const pubResult = await publisher.publish("Test.Retry", { foo: "retry" });
      assertEquals(pubResult.isOk(), true);

      await waitFor(() => attempts >= 2, { description: "event redelivery after nak" });
      assertEquals(attempts >= 2, true);

      await subNc.drain();
    });

    await t.step("invalid payload is terminated (no handler call)", async () => {
      const subNc = await connect({ servers: `localhost:${info.port}` });
      const subscriber = createClient(
        contract,
        subNc,
      ...
      await pubNc.publish(
        ...
        JSON.stringify({ header: { id: "x", time: "2026-01-01T00:00:00.000Z" } }),
      );
