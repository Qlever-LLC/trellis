...
const forbiddenImportPattern = /(?:from|require\()\s*["']@qlever-llc\/trellis-[^"']+["']/;
...
Deno.test("trellis npm artifact only depends on allowed published Trellis packages", async () => {
  ...
  assertEquals(Object.keys(packageJson.dependencies).some((name: string) => name.startsWith("@qlever-llc/trellis-")), false);
