...
function unwrapOk<T, E extends BaseError>(
  result: Result<T, E>,
  message: string,
): T {
  return result.match({
...
Deno.test({
  ...
  async fn(t) {
    ...
    await t.step(
      "get() returns ValidationError and preserves the raw entry",
      async () => {
        await kv.kv.put(
          "invalid-get",
          JSON.stringify({ name: "missing-count" }),
        );
    ...
    await t.step(
      "watch() emits an error event and preserves the raw entry",
      async () => {
        const key = "invalid-watch";
        const createResult = await kv.create(key, {
          name: "initial",
          count: 1,
        });
    ...
    await t.step(
      "TypedKVEntry.create() returns ValidationError and preserves the raw entry",
      async () => {
        ...
        assertExists(
          preserved,
          "Invalid entry should remain present after validation failure",
        );
      },
    );
  },
...
Deno.test({
  ...
  async fn(t) {
    ...
    await t.step(
      "watch() calls callback on updates with decoded values",
      async () => {
        // Create an entry
        ...
        const updateEvent = events.find((e) =>
          e.type === "update" && e.value?.count === 42
        );
    ...
    await t.step(
      "watch() calls callback on deletes when includeDeletes is true",
      async () => {
        // Create an entry
...
Deno.test({
  ...
  async fn(t) {
    ...
    await t.step(
      "CAS delete succeeds with correct revision and then fails on reuse",
      async () => {
