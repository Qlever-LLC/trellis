...
>
  ...
  <div
    class="card w-full max-w-sm border border-base-300 bg-base-100 shadow-md"
  >
      ...
      {:else if flow.state?.status === "approval_required"}
        ...
        <div class="rounded-box border border-base-300 bg-base-100 p-4">
          ...
          <p class="mt-1 text-sm font-semibold text-base-content">
            {flow.state.user.name ?? flow.state.user.id}
          </p>
          <p class="mono mt-0.5 text-xs text-base-content/50">
      ...
      {:else if flow.state?.status === "insufficient_capabilities"}
        <div>
          ...
          <p class="mt-1 text-sm text-base-content/60">
            ...
            {:else}
              Your account is missing required capabilities. Return to the CLI
              to finish sign-in or close this page.
            {/if}
          </p>
        </div>
        {#if flow.state.user}
        ...
        {#if shouldShowReturnToAppLink()}
          <a class="btn btn-outline btn-block" href={returnToApp()}
            >Return to app</a
          >
        {/if}
