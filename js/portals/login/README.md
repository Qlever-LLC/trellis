...
Trellis-owned SvelteKit portal for browser auth UX and the default workload
activation route.
...
- `/_trellis/portal/activate` resumes a preserved `handoffId` after sign-in and
  starts `Auth.ActivateWorkload` over the Trellis runtime.
...
activation route starts the workload activation RPC over the Trellis runtime.
