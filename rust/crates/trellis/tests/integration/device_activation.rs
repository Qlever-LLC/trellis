...
#[tokio::test]
#[ignore]
async fn device_activation_device_client_activates_and_connects() {
    ...
    // Build and approve device contract
    ...
    // Derive device identity and provision the device
    ...
    // Resolve activation via admin operation
    let resolve_op = auth
        .operation()
        .auth()
    ...
    // Wait for device activation via HTTP polling
    ...
    // Verify device can flush to confirm NATS connection is live
    device
        .flush()
        .await
        .expect("device NATS flush should succeed");

    // Use GeneratedAuthClient for typed RPC call to Auth.Sessions.Me
    ...
    // List device user authorities and verify the activated device is present
    let listings = auth
        .rpc()
        .auth()
