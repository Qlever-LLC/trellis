...
pub struct LocalTrellisBootstrapPaths {
    ...
    /// Local GitHub OAuth client secret file path.
    pub github_oauth_secret: String,
...
pub fn generate_local_trellis_bootstrap(
    ...
    fs::create_dir_all(trellis_out.join("oauth"))?;
    ...
    fs::write(
        trellis_out.join("oauth/github.secret"),
        "local-dev-github-secret\n",
    )?;
...
pub fn render_trellis_config(
    ...
    format!(
        r#"{{
  // Generated by `trellis bootstrap local` for local development.
  "port": {trellis_port},
  "web": {{
  ...
  "oauth": {{
    ...
    "providers": {{
      "github": {{
        "type": "github",
        "clientId": "local-dev",
        "clientSecretFile": "./oauth/github.secret"
      }}
    }}
  }}
}}
...
mod tests {
    ...
    fn trellis_config_uses_expected_local_paths_and_urls() {
        ...
        assert!(config.contains("\"clientId\": \"local-dev\""));
        assert!(config.contains("\"clientSecretFile\": \"./oauth/github.secret\""));
    }
    ...
    fn trellis_manifest_omits_secret_values() {
        ...
        assert!(json.contains("github.secret"));
        assert!(!json.contains("local-dev-session-seed"));
        assert!(!json.contains("local-dev-github-secret"));
        assert!(!json.contains("SA"));
        assert!(!json.contains("SX"));
    }
