<script lang="ts">
  ...
  let note = $state("West Yard · Pump Station 7\nObserved minor vibration during the morning walk-through. Follow-up evidence attached from the browser console.");
  let evidenceType = $state("operator-note");
  ...
  async function startUpload(): Promise<void> {
    ...
    try {
      ...
      const upload = await trellis.operation("Evidence.Upload")
        .input({ key, contentType: "text/plain", evidenceType })
        .transfer(bytes)
...
<svelte:head>
  <title>Evidence · Field Ops Console</title>
</svelte:head>

<section class="flex w-full flex-col gap-6">
  <header class="space-y-1">
    <h1 class="text-2xl font-semibold">Evidence</h1>
    <p class="text-sm text-base-content/70">Upload a text payload as field evidence.</p>
    <div class="badge badge-outline">Uses: transfer operation</div>
  </header>
    ...
    <section class="card border border-base-300 bg-base-100 shadow-sm">
      <div class="card-body gap-4">
        <div class="flex items-center justify-between gap-3">
          <h2 class="card-title text-lg">Upload status</h2>
          {#if acceptedId}
        ...
        {#if progressLog.length === 0}
          <div class="alert"><span>Start an upload to see transfer and operation progress.</span></div>
        {:else}
